	...
	<div class="row">
		...
		<div class="content">
			...
			<div id="<?php echo user_can_richedit() ? 'postdivrich' : 'postdiv'; ?>" class="postarea">
				<?php wp_editor(stripslashes($content), $value['id'], $value['id'], true); ?>
			</div>
			<span class="hint"><?php if(isset($value['desc'])) { echo $value['desc']; } ?></span>
		</div>
	</div>
	...
	<div class="row">
		...
		<div class="content">
			...
			<div class="typography_preview">
				<div style="font-family: <?php echo "'$font'"; ?>; font-size: <?php if(get_option( $value['id'].'_size') != "") { echo get_option($value['id'].'_size'); } else { echo $value['font_size'].'px'; } ?>; color: <?php if(get_option( $value['id'].'_color') != "") { echo get_option($value['id'].'_color'); } else { echo "#333333"; } ?>;"><?php echo $value['name']; ?></div>
			</div>
			...
			<script type="text/javascript" charset="utf-8">
				jQuery(document).ready(function() {
					...
					<?php if($value['id'] == "fa_logo_font" && get_option($shortname.'_logo_type') != "text") { ?>
...
?>
	...
	<div class="row">
		...
		<div class="content check">
				...
				?>
				<select name="<?php echo $value['id']; ?>" class="icon general-enclosed">
					<option value=""<?php if(get_option( $value['id']) == ''){ echo " selected";} ?>></option>
					<?php foreach($icon_options as $option) { ?>
					<option value="<?php echo $option; ?>" <?php if(get_option( $value['id']) == $option) { echo "selected"; } ?>><?php echo $option; ?></option>
					<?php } ?>
				</select>
			<span class="hint"><?php echo $value['desc']; ?></span>
		</div>
